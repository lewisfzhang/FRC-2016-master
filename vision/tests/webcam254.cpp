...
bool SetAndCheckCameraSettings(const v4l2_control& control, int v4l_descriptor) {
	...
	while (num_retries_left > 0) {
		if (v4l2_ioctl(v4l_descriptor, VIDIOC_G_CTRL, &c) == 0) {
			if (c.value != control.value) {
				...
				std::cout << "Property " << c.id << " already at the desired value" << std::endl;
				return true;
			}
		} else {
...
void App::run()
{
	...
	c.value = 10;
	SetAndCheckCameraSettings(c, descriptor);Parameters:
	...
	if(!(capabilities.capabilities & V4L2_CAP_VIDEO_CAPTURE) || !(capabilities.capabilities & V4L2_CAP_STREAMING)) {
	...
	// For some reason, the c920 camera tends to reset exposure upon beginning streaming
	...
	while (running_)
	{
		...
		frame = cv::imdecode(cv::Mat(1080, 1920, CV_8UC3, buffer_start), CV_LOAD_IMAGE_COLOR);

		//vc >> frame;
		cv::cvtColor(frame, frame_hsv, cv::COLOR_BGR2HSV);

		// Define range of blue color in HS
		cv::inRange(frame_hsv, cv::Scalar(52,117,76), cv::Scalar(101,255,255), frame_threshold);
		
		// Morphology (erode + dilate)
		cv::morphologyEx(frame_threshold, frame_morphology, cv::MORPH_OPEN, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(15, 15)), cv::Point(-1, -1), 1);
		cv::morphologyEx(frame_morphology, frame_morphology2, cv::MORPH_CLOSE, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(15, 15)), cv::Point(-1, -1), 1);
		
		// Contours (finding edges)
		cv::findContours(frame_morphology2, contours, hierarchy, cv::RETR_TREE, cv::CHAIN_APPROX_SIMPLE, cv::Point(0,0));
		...
		for (int i = 0; i < contours.size(); i++)
		{
			...
			//Find moments- Average of image pixel intensities, used to find center of mass
			cv::moments(contours[i], false);
			cv::Point centerOfMass = (moment['m10']/moment['m00'], moment['m01']/moment['m00']);
			cv::drawContours(frame_morphology2, hull, -1, blue, 3, 8, hierarchy, 0, cv::Point());
