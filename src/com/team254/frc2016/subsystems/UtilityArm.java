...
public class UtilityArm extends Subsystem {
    ...
    /** If there's a bug, revert to portcullis mode, it's the least likely to break the robot */
    ...
    private Solenoid mAdjustableHardStopSolenoid =
            Constants.makeSolenoidForId(Constants.kAdjustableHardStopSolenoidId);
    private Solenoid mCdfFlapSolenoid = Constants.makeSolenoidForId(Constants.kCdfFlapSolenoidId);
    private Solenoid mHookReleaseSolenoid =
            Constants.makeSolenoidForId(Constants.kHookReleaseSolenoidId);
    private Solenoid mGasSpringReleaseSolenoid =
            Constants.makeSolenoidForId(Constants.kGasSpringReleaseSolenoidId);
    ...
    private synchronized SystemState handleSizeBox() {
        return mWantedState == WantedState.STAY_IN_SIZE_BOX
                ? SystemState.SIZE_BOX
    ...
    private synchronized SystemState handleSizeBoxToPortcullis(double timeSinceStateStart) {
        ...
        return timeSinceStateStart >= Constants.kUtilityArmSizeBoxToPortcullisDelay
    ...
    private synchronized SystemState handleLiftingArmForHang(double timeSinceStateStart) {
        ...
        return timeSinceStateStart >= Constants.kUtilityArmLiftForHangToOpenCdfDelay
    ...
    private synchronized SystemState handleOpeningCdfForHang(double timeSinceStateStart) {
        ...
        return timeSinceStateStart >= Constants.kUtilityArmOpenCdfToDeployHooksDelay
                ? SystemState.DEPLOY_HOOKS
                : SystemState.OPENING_CDF_FOR_HANG;
    }
        ...
        switch (systemState) {
            ...
            case DEPLOY_HOOKS:
                setOutputs(
                        ...
                        GasSpringReleaseOutput.STOWED,
                        IntakeDeployRequirements.FREE); // TODO: add an intake up control here?
                break;
            case HANG:
                setOutputs(
                        ArmOutput.ARM_UP,
    ...
    /**
     * Always set all the outputs so we aren't tempted to merge states of the solenoid outputs
     * (which lose state on disable) and this state machine (which keeps state through disable).
     */
    private void setOutputs(
            ...
            IntakeDeployRequirements intakeDeployRequirements) {
        ...
        mIntake.setDeploySafetyRequirement(
                intakeDeployRequirements == IntakeDeployRequirements.FORCE_DEPLOYED);
    }
